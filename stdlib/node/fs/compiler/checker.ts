...
import { isFsConstantValue } from '../../../../compiler/checker/builtins.ts'
import type { FsRuntimeCallInfo } from './descriptor.ts'
import {
...
export function fsRuntimeCallInfoFromImportSymbol(
  ...
  if (!isFsRuntimeImportSource(symbol.importSource)) {
    return null
  }

  if (symbol.importedName === null || typeof symbol.importedName === 'undefined') {
    return null
  }
...
export function isFsPromisesImportSymbol(symbol: SymbolInfo | null | undefined): boolean {
  ...
  return symbol.importSource === 'node:fs' && symbol.importedName === 'promises'
}
...
function isFsRuntimeImportSource(source: string | null | undefined): boolean {
  return source === nodeFsImportSource || source === nodeFsPromisesImportSource
}
