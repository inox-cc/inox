...
function formatGeneratedC(code: string, _assumeFilename: string): string {
  ...
  return [
    {
      ...
      code: formatGeneratedC(emitCModuleSource(plan, plans, options, diagnostics), plan.sourcePath)
    },
    {
      kind: 'header',
      path: plan.headerPath,
      sourcePath: plan.record.path,
      code: formatGeneratedC(emitCModuleHeader(plan, plans, diagnostics), plan.headerPath)
    }
  ]
}
