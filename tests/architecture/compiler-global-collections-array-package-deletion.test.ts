...
const fixtureRoot = resolve('dist/test-tmp/compiler-global-collections-array-package-deletion')
const outputRoot = resolve(fixtureRoot, 'dist/compiler-libraries')
const source = "const values = Array.from('ab')\nvalues.push('c')\n"

test('удаление global:collections убирает Array API и native plan', async () => {
  await createFixture()
