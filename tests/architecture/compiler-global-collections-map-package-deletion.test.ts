...
const fixtureRoot = resolve('dist/test-tmp/compiler-global-collections-map-package-deletion')
const outputRoot = resolve(fixtureRoot, 'dist/compiler-libraries')
const source = "const values = new Map<string, number>()\nvalues.set('x', 1)\n"

test('удаление global:collections убирает Map API и native plan', async () => {
  await createFixture()
