...
test('global:promise владеет declarations, operations, async-result provider и runtime plan', async () => {
  ...
  const result = compileSource(
    `
      ...
      const combined = Promise.all([Promise.resolve(5), 6])
      const raced = Promise.race([Promise.resolve(7), 8])
      const finalized = resolved.finally(() => {})
    `,
    { libraries, target: 'cc' }
  )
  ...
  assert.equal(result.ast.body[5].init.libraryOperationId, 'global:promise#Promise.all')
  assert.equal(result.ast.body[6].init.libraryOperationId, 'global:promise#Promise.race')
  assert.equal(result.ast.body[7].init.libraryOperationId, 'global:promise#Promise.finally')
  ...
  assert.equal(result.ast.body[5].init.typeRef.args[0].typeId, 'global:collections#Array')
  assert.equal(result.ast.body[5].init.typeRef.args[0].args[0].name, 'number')
  assert.equal(result.ast.body[6].init.typeRef.args[0].name, 'number')
