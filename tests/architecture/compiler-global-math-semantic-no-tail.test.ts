...
const forbiddenPatterns = [
  /stdlib\/global\/math\/compiler\/(?:checker|descriptor)/,
  /\b(?:check|emit|is|known)\w*Math\w*\b/,
  /\bmathRuntime\w*\b/,
  /\bneedsMathRuntime\b/,
  /['"]Math['"]/
]

test('portable compiler не содержит target Math semantic tails', async () => {
  const tails: string[] = []
  ...
  assert.deepEqual(
    {
      packageCompilerFiles: (await readdir('stdlib/global/math/compiler')).sort(),
      tails
    },
    {
