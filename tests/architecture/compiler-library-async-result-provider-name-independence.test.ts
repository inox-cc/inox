...
`,
  ...
  assert.match(result.code, /fixture::FutureTask work\(\)/)
  assert.match(result.code, /co_await fixture::FutureTask::bridgeAwait\(/)
  assert.doesNotMatch(result.code, /inox_async_task_/)
  assert.match(result.code, /inox::String\(inox::object_value_at\(reason, 1, "message"\)\)/)
  assert.doesNotMatch(result.code, /auto inox_value_\d+ = inox::object_value_at\(reason, 1, "message"\);/)
