...
test('C contexts сохраняют dependency shape через generic boundary', () => {
  ...
  const declarationsSource = readFileSync(new URL('../../compiler/c/declarations.ts', import.meta.url), 'utf8')
  const indexSource = readFileSync(new URL('../../compiler/c/index.ts', import.meta.url), 'utf8')
  const unitSource = readFileSync(new URL('../../compiler/c/unit.ts', import.meta.url), 'utf8')
