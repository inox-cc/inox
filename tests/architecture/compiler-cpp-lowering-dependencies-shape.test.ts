...
test('C++ contexts сохраняют dependency shape через generic boundary', () => {
  ...
  assert.doesNotMatch(contextSource, /AsyncTaskDependencies|CoroutineDependencies/)
  assert.doesNotMatch(indexSource, /AsyncTaskLoweringDependencies|AsyncCoroutineLoweringDependencies/)
