...
test('entrypoint package node:child_process владеет sync operations и runtime plan', async () => {
  ...
  const execFileSync = operations.find(
    (operation) => operation.operationId === 'node:child_process#execFileSync'
  )
  const spawnSync = operations.find(
    (operation) => operation.operationId === 'node:child_process#spawnSync'
  )
  ...
  assert.ok(
    execFileSync?.bindingAliases?.includes(
      'node:child_process#module:node:child_process:default.execFileSync'
    )
  )
  assert.deepEqual(spawnSync?.resultShapeFields, [
    { name: 'status', valueType: 'number', readonly: true },
    { name: 'stdout', valueType: 'string', readonly: true },
    { name: 'stderr', valueType: 'string', readonly: true }
  ])
