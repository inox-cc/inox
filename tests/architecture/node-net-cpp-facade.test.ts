...
test('node:net использует JS-shaped RAII C++ facade без legacy wrappers', async () => {
  ...
  assert.match(header, /NetServer& getConnections\(inox::Callback callback\);/)
  assert.match(header, /bool listening\(\) const;/)
  assert.match(header, /inox::Value maxConnections\(\) const;/)
  assert.match(header, /void setMaxConnections\(double maximum\);/)
  ...
  assert.match(header, /NetSocket& setTimeout\(double timeout, inox::Callback callback\);/)
