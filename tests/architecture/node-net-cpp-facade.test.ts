...
test('node:net использует JS-shaped RAII C++ facade без legacy wrappers', async () => {
  ...
  assert.match(header, /bool listening\(\) const;/)
  assert.match(header, /NetServer& ref\(\);/)
  assert.match(header, /NetServer& unref\(\);/)
  ...
  assert.match(header, /bool connecting\(\) const;/)
  assert.match(header, /bool destroyed\(\) const;/)
  assert.match(header, /bool isPaused\(\) const;/)
  assert.match(header, /NetSocket& pause\(\);/)
  assert.match(header, /bool pending\(\) const;/)
  assert.match(header, /inox::String readyState\(\) const;/)
  assert.match(header, /NetSocket& resume\(\);/)
