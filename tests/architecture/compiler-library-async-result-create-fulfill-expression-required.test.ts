...
test('async-result create operation требует непустой fulfill expression', () => {
  ...
  assert.ok(operation)
  operation.cAsyncFulfillExpression = '   '

  assert.throws(() => createCompilerLibrarySet([library]), {
    message: `Compiler library intrinsic provider async-result create operation ${operation.operationId} requires non-empty cAsyncFulfillExpression`
  })
})
