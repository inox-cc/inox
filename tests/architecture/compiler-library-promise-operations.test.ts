...
test('data-only library Promise operations preserve fulfilled metadata and lower generically', () => {
  ...
  assert.match(result.code, /auto inox_await_\d+ = Array\(fixture\.list\(\)\.awaitValue\(\)\);/)
  assert.doesNotMatch(result.code, /inox_library_asyncResult_\d+/)
  assert.doesNotMatch(result.code, /auto inox_await_converted_\d+ = Array\(inox_await_\d+\);/)
