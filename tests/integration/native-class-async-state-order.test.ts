...
`
  ...
  const runDefinitionIndex = source.indexOf('static void run(void) {')
  ...
  assert.notEqual(flagIndex, -1, 'missing unhandled rejection state')
  ...
  assert.ok(methodIndex < flagIndex, 'runtime state should not split class declarations from method definitions')
  assert.ok(flagIndex < runDefinitionIndex, 'runtime state should stay before functions that use it')
