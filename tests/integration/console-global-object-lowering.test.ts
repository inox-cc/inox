...
`
  ...
  assert.match(source, /auto inox_library_result_\d+ = Object\.entries\(parsed\);/)
  assert.match(source, /console\.log\("entries %s %s %s", inox_library_result_\d+, process\.versions, process\);/)
