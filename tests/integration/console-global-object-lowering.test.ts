...
`
  ...
  assert.doesNotMatch(source, /auto inox_library_object_\d+ = Object\.entries\(parsed\);/)
  assert.match(source, /console\.log\("entries %s %s %s", Object\.entries\(parsed\), process\.versions, process\);/)
  assert.match(
    source,
    /console\.log\("entries %s %s %s", Object\.entries\(parsed\), process\.versions, process\);\n  if \(inox::thrown\(\)\) return;/
  )
