...
`
  ...
  assert.match(source, /auto inox_await_value_\d+ = inox_async_result_\d+\.awaitValue\(\);/)
  assert.match(
    source,
    /if \(inox::thrown\(\)\) goto catch_\d+;/
  )
  assert.match(source, /auto (?:error|inox_error) = inox::take_exception\(\);/)
  assert.match(source, /inox_async_result_\d+ = inox::Promise::reject\(inox::String\("bad", 3\)\);/)
