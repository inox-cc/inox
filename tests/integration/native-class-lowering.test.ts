...
`
  ...
  assert.match(result.code, /static inox_status inox_read_field\(const Box& value, uint32_t index, inox_value\* out\)/)
  assert.doesNotMatch(result.code, /Box::inox_read_field/)
...
`
  ...
  assert.match(result.code, /#include "inox\/map\.h"/)
  assert.match(result.code, /\n\s*Map scores;/)
  assert.match(result.code, /this->scores\.size\(\)/)
  assert.doesNotMatch(result.code, /\n\s*inox::Value scores;/)
  assert.doesNotMatch(result.code, /Map\(this->scores\.raw\(\)\)/)
  assert.doesNotMatch(result.code, /this->scores\.raw\(\)/)
...
`
  ...
  assert.match(result.code, /Foo\(const inox::String& name\) : name\(name\) \{\n\s*\}/)
  assert.doesNotMatch(result.code, /Foo\(const inox::String& name\);/)
  assert.doesNotMatch(result.code, /Foo::Foo/)
  ...
  const constructor = generatedBlock(result.code, 'Foo(const inox::String& name)')
