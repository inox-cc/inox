...
`
  ...
  assert.match(source, /auto inox_library_result_\d+ = Object\.entries\(Array\(inox_value_\d+\)\);/)
  assert.match(source, /auto inox_library_result_\d+ = Array\(inox_library_result_\d+\)\.get\(0\);/)
