...
`
  ...
  assert.match(source, /regexp\.test\(static_cast<inox::StringView>\(trimmed\)\)/)
  assert.match(source, /auto inox_library_object_\d+ = RegExp\("Inox", ""\);/)
  assert.match(source, /inox_library_object_\d+\.test\(static_cast<inox::StringView>\(trimmed\)\)/)
  assert.match(source, /console\.log\("%d", regexp\.test\(static_cast<inox::StringView>\(trimmed\)\)\);/)
