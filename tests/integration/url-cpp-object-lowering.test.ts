...
export function assertUrlRuntimeUsesStringFacade(): void {
  ...
  assert.doesNotMatch(header, /static URL from\([^)]*inox_shape/)
  ...
  assert.doesNotMatch(header, /pathToFileURL\([^)]*inox_shape/)
...
`
  ...
  assert.doesNotMatch(source, /inox_shape_library_result_/)
