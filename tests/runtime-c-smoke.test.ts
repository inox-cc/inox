...
console.log(user.name === same, user['name'] !== other, user.name === other)
...
console.log(
  isLiteral({ args: [{ type: 'Literal' }] }),
  isLiteral({ args: [{ type: 'Other' }] }),
  isLiteral({ args: [] })
)
...
test('generated C Record object variable key reads compile and run with runtime sources', async (t) => {
  const probe = await runCommand('cc', ['--version'])
