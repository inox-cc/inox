...
const allowedHostImports = new Map<string, Set<string>>([
  ['node:crypto', new Set(['compiler/node-host.ts'])],
  ['node:fs', new Set(['compiler/node-host.ts'])],
  ['node:fs/promises', new Set(['compiler/node-host.ts'])],
  ['node:path', new Set(['compiler/node-host.ts'])],
  ['node:url', new Set(['compiler/node-host.ts'])]
])
...
function renderMatrix(report: AuditReport): string {
  const lines: string[] = [
    ...
    '',
    'This matrix makes the self-hosting blockers in `compiler` measurable. It',
    ...
    '3. Extract the remaining large C emitter clusters before trying to compile `compiler/c/index.ts` with inox.',
